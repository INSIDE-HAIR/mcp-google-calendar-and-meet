...
async function testGoogleMeetMCPServer() {
  ...
  try {
    ...
    const betaTests = [
      {
        name: 'createSpaceMember (not available)',
        test: () => Promise.resolve({ message: 'Method not available in current API' }),
        description: 'Add member to space (beta)'
      },
      {
        name: 'listSpaceMembers (not available)',
        test: () => Promise.resolve({ members: [] }),
        description: 'List space members (beta)'
      },
      {
        name: 'getSpaceMember (not available)',
        test: () => Promise.resolve({ message: 'Method not available in current API' }),
        description: 'Get space member details (beta)'
      },
      {
        name: 'deleteSpaceMember (not available)',
    ...
    for (const test of betaTests) {
      try {
        const result = await test.test();
        if (result && typeof result === 'object' && 'betaDisabled' in result && (result as any).betaDisabled) {
    ...
    const featureFlags = (api.meetRestClient as any)?.baseUrls || {};
    console.log(`  🚫 Beta features disabled: ${!(featureFlags as any).enableV2BetaFeatures ? '✅' : '❌'}`);
    console.log(`  🚫 WebRTC disabled: ${!(featureFlags as any).enableExperimentalWebRTC ? '✅' : '❌'}`);
